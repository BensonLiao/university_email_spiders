...
class MITSpider(CrawlSpider):
    ...
    accept_keywords = ''.join([
        'people|faculty|staff|person'
    ])

    accept_xpath = ''.join([
        '//a[contains(text(), "people")',
        ' or contains(text(), "People")',
        ' or contains(text(), "faculty")',
        ' or contains(text(), "Faculty")',
        ' or contains(text(), "staff")',
        ' or contains(text(), "Staff")',
        ']'
    ])
    ...
    except_keywords = ''.join([
        'login|news|event|campaign|media|press|spotlight|community|libra|student|regist|graduate|job|life|contact',
        '|financial|fact|story|map|site|calendar|catalog|commencement|career|copyright|issue|help|search|video',
        '|watch|terms|rss|feed|licens|sponsor|overview|committee|meet|faq|question|channel|wiki|label',
        '|visit|give|giving|print',
